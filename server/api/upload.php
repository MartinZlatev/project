...
if (isset($_POST['submit'])){
    ...
    if (in_array($fileExt, $allowed)) {
        if ($fileError === 0) {
            ...
            if (!isset($json_assoc[$hash])) {
                ...
                $sqlInsertImage = "INSERT INTO images (path, original_filename, number_instances) 
                                   VALUES (?, ?, '1')";

                $imageInsertStatement = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($imageInsertStatement, $sqlInsertImage)) {
                    header("Location: ../../client/home-page?error=sqlerror3");
                    exit();
                } else {
                    // Create an Image
                    mysqli_stmt_bind_param(
                        $imageInsertStatement, "ss", 
                        $filenameNew, 
                        $originalFilename,
                    );
            ...
            //      Increase number_instances of the Image
                ...
                if (!mysqli_stmt_prepare($imageGetStatement, $sqlGetImage)) {
                    header("Location: ../../client/home-page?error=sqlerror4");
                    exit();
                } 
                else {
            ...
            header("Location: ../../client/home-page?success=uploaded");
            mysqli_close($conn);
    ...
    header("Location: ../../client/home-page?ebaloSieMaikata");
}